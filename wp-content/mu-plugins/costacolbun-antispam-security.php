<?php
/**
 * Plugin Name: Costa Colbun - Anti-Spam & Security Maximum
 * Description: Proteccion maxima contra spam, virus, bots y ataques para costacolbun.cl.
...
add_filter('wpcf7_form_elements', function ($content) {
    ...
    $honeypot .= '<label for="costacolbun_website_url">Website</label>';
    $honeypot .= '<input type="text" name="costacolbun_website_url" id="costacolbun_website_url" value="" tabindex="-1" autocomplete="off" />';
    $honeypot .= '<input type="text" name="costacolbun_phone_confirm" id="costacolbun_phone_confirm" value="" tabindex="-1" autocomplete="off" />';
    $honeypot .= '</div>';
...
add_filter('wpcf7_spam', function ($spam, $submission) {
    ...
    if (!empty($_POST['costacolbun_website_url']) || !empty($_POST['costacolbun_phone_confirm'])) {
        $submission->add_spam_log(array(
            'agent'  => 'costacolbun_honeypot',
...
add_filter('wpcf7_spam', function ($spam, $submission) {
    ...
    $ip = costacolbun_get_client_ip();
    $transient_key = 'costacolbun_cf7_rate_' . md5($ip);
...
add_filter('wpcf7_spam', function ($spam, $submission) {
    ...
    if (preg_match('/(\r|\n|%0a|%0d|bcc:|cc:|to:|content-type:|mime-version:|subject:)/i', $all_content)) {
        $submission->add_spam_log(array(
            'agent'  => 'costacolbun_header_injection',
    ...
    foreach ($spam_keywords as $keyword) {
        if (strpos($content_lower, strtolower($keyword)) !== false) {
            $submission->add_spam_log(array(
                'agent'  => 'costacolbun_keyword_spam',
    ...
    if ($cyrillic_chars > 0 && $cyrillic_chars > $latin_chars) {
        $submission->add_spam_log(array(
            'agent'  => 'costacolbun_language_spam',
...
// =============================================
add_filter('wpcf7_form_elements', function ($content) {
    $timestamp = '<input type="hidden" name="costacolbun_form_time" value="' . esc_attr(time()) . '" />';
...
add_filter('wpcf7_spam', function ($spam, $submission) {
    ...
    if (isset($_POST['costacolbun_form_time'])) {
        $form_time = intval($_POST['costacolbun_form_time']);
        $elapsed = time() - $form_time;

        if ($elapsed < 3 && $elapsed >= 0) {
            $submission->add_spam_log(array(
                'agent'  => 'costacolbun_time_check',
    ...
    if (!empty($args['headers'])) {
        ...
        foreach ($args['headers'] as $header) {
            ...
            if (preg_match('/^(bcc|cc)\s*:/i', $header)) {
                if (strpos($header, '@costacolbun.cl') !== false) {
...
add_filter('authenticate', function ($user, $username, $password) {
    ...
    $ip = costacolbun_get_client_ip();
    $transient_key = 'costacolbun_login_attempts_' . md5($ip);
    ...
    if ($attempts >= 5) {
        return new WP_Error(
            'costacolbun_too_many_attempts',
            '<strong>Seguridad Costa Colbun:</strong> Demasiados intentos fallidos. Por favor espere 30 minutos antes de intentar nuevamente.'
        );
    }
...
// HELPER FUNCTIONS
// =============================================
function costacolbun_get_client_ip() {
    $ip_keys = array(
...
add_filter('wpcf7_spam', function ($spam) {
    if ($spam) {
        ...
        error_log(sprintf(
            '[Costa Colbun AntiSpam] SPAM BLOCKED | IP: %s | UA: %s | Time: %s',
            $ip,
            substr($ua, 0, 100),
            current_time('mysql')
        ));
